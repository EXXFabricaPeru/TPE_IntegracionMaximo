...
{
    public class CostAdjustmentsProcess
    {

        private static readonly log4net.ILog logger = log4net.LogManager.GetLogger(typeof(CostAdjustmentsProcess));
        internal static void Process(ref Company oCompany, List<ConfigClass> listConfig)
        {
            try
            {
                var listSN = GetCosts(oCompany, listConfig);
                var url = listConfig.Where(t => t.Code == Constants.URL_COST).FirstOrDefault().Value;
                ...
                foreach (var item in listSN)
                {
                    ...
                    ////else
                    UpdateCost(oCompany, item, listConfig, url, maxAuth, authorization);
        ...
        private static void SendOrder(Company oCompany, CostClass item, List<ConfigClass> listConfig, string url, string maxAuth, string authorization)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        //UpdateCost(oCompany, item, listConfig);
        ...
        private static void UpdateCost(Company oCompany, CostClass item, List<ConfigClass> listConfig, string url, string maxAuth, string authorization)
        {
            try
            {
                ...
                {
                    var resp = JsonConvert.DeserializeObject<CompaniesResponseClass>(response.Content);
                    logger.Info("UpdateCost: Respuesta " + response.Content);
                ...
                else
                {
                    ...
                    {
                        //UpdateCost(oCompany, item, listConfig);
                        logger.Error("UpdateCost " + response.Content);
        ...
        private static void UpdateState(CostClass item, Company oCompany, string state, string message, string id = "")
        {
        ...
        private static List<CostClass> GetCosts(Company oCompany, List<ConfigClass> listConfig)
        {
...
";
                ...
                while (!recordset.EoF)
                {
                    ...
                    //_invcost.conditioncode= (string)recordset.Fields.Item("location").Value;
